	...
	namespace ShaderCompiler
	{
		...
		bool CompileShader(ShaderCache& shader_cache, const ShaderProgramInfo::ShaderData& shader_data, CompilationResult& out_result)
		{
			...
			struct IncludeHandler : public IDxcIncludeHandler
			{
				...
				{
					std::string filename = (LPCSTR)CW2A(pFilename);
				...
				std::unordered_map<std::string, std::unique_ptr<Blob>> blobs;
			};

			auto& shader_source = shader_cache.GetShaderSource(shader_data.path);
			if (!shader_source.size())
			{
				out_result.error = "Can't open shader source file " + shader_data.path;
				return false;
			}
			...
			auto entry_point = StringToWString(shader_data.entry_point);
			auto shader_path = StringToWString(shader_data.path);
			...
			for (auto& define : shader_data.defines)
			{
				stream.clear();
				stream.str(L"");
				stream << StringToWString(define.name) << L"=" << StringToWString(define.value);
				defines.push_back(stream.str());
			}

			std::vector<LPCWSTR> args;
			args.push_back(shader_path.c_str());
