...
namespace {
    ...
    VkExtent2D chooseSwapExtent(VkSurfaceCapabilitiesKHR const& capabilities) {
        if (capabilities.currentExtent.width != UINT32_MAX) {
            ...
            auto const window = std::dynamic_pointer_cast<donut::vulkan::GLFWWindow>(donut::Application::GetInstance()->GetWindow());
            auto const glfwWindow = window->GetGLFWWindow();
    ...
    void createGraphicsPipeline() {
        auto vertShaderCode = readFile("src/renderer/renderer_apis/vulkan/vert.spv");
        auto fragShaderCode = readFile("src/renderer/renderer_apis/vulkan/frag.spv");
